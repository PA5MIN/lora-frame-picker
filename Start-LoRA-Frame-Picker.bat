...
".venv\Scripts\python.exe" -c "import cv2;from PIL import Image;import flask" >nul 2>nul
if errorlevel 1 (
  echo Installing OpenCV, Pillow and Flask...
...
".venv\Scripts\python.exe" lora_frame_picker.py
...
goto pause_error
:tkinter_missing
echo Tkinter is missing. Reinstall Python from python.org and include Tcl/Tk.
goto pause_error
:app_failed
echo The application exited with an error. Copy the full message above into a GitHub Issue.
:pause_error
pause
exit /b 1
